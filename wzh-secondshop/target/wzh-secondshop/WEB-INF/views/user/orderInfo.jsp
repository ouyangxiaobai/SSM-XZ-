			...
			<c:choose>
				...
				<c:otherwise>
					<c:forEach var="order" items="${orders}">
						<a href="/user/orderInfo?orderId=${order.id}">
...
<script>
    ...
    function deleteOrder() {
        $.getJSON("/user/order/delete/" + ${orderInfo.id}, function (data) {
            if (data == true){
                alert("订单删除成功！");
                $(window).attr('location','/user/userProfile');
            } else {
                alert("订单删除失败！");
            }
        })
    ...
    function selectValue() {
        $.getJSON("/user/order/update/status/" + ${orderInfo.id} + "&3", function (data) {
            if (data == false){
                alert("操作失败！");
            } else {
