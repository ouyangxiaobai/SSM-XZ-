...
<body>
		...
		<div class="col-md-9" style="float: left">
			...
			<div class="col-md-12 r2"
				...
				<div class="col-md-10">
					...
					<div class="col-md-3">
						<a
							href="/user/userEdit?userId=${sessionScope.user.getId()}">修改个人资料</a>
					</div>
				</div>
			</div>
										...
										<c:forEach var="collect" items="${collects}">
											<tr style="color: #666666">
												<td style="width: 102px"><a target="_blank"
													href="/goods/goodInfo?goodId=${collect.good.id}">${collect.good.id}</a></td>
								...
								<table class="table table-hover">
									<tbody>
										<c:forEach var="good" items="${goods}">
											<tr style="color: #666666">
												<td style="width: 102px"><a target="_blank"
													href="/goods/goodInfo?goodId=${good.id}">${good.id}</a></td>
										...
										<c:forEach var="order" items="${orders}">
											<tr style="color: #666666">
												<td style="width: 90px"><a target="_blank"
													href="/user/orderInfo?orderId=${order.id}">${order.id}</a></td>
					...
					<c:choose>
						...
						<c:otherwise>
							<c:forEach var="review" items="${reviews}">
								<a target="_blank"
									href="/user/review?goodId=${review.goodId}&reviewId=${review.id}">
					...
					<c:choose>
						...
						<c:otherwise>
							<c:forEach var="reply" items="${replies}">
								<a target="_blank"
									href="/user/reply?reviewId=${reply.reviewId}&replyId=${reply.id}">
									<div class="col-md-12 rev"
										style="padding-left: 0px; padding-right: 0px; margin-bottom: 5px;">
										<div class="col-md-11">
											<p></p>
						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach var="sellGood" items="${sellGoods}">
									<a
										href="/user/sellerInfo?orderId=${sellGood.id}">
										<div class="col-md-12 rev"
    ...
    function deleteCollect(collectId) {
        $.get("/collect/delete/"+collectId+"&"+${sessionScope.user.id}, function (data) {
            if (data === false){
                alert("未知原因，删除失败！");
            } else {
                ...
                $.each(data, function (i, type) {
                    ...
                    var collect = "<tr style=\"color: #666666\">"+
                        "<td style=\"width: 102px\"><a target='_blank' href='/goods/goodInfo?goodId="+collectGoodId+"'>"+collectGoodId+"</a></td>"+
